...
class FcoDailySpecial_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'fco-daily-special'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'fco-daily-special'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'fco-daily-special'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'fco-daily-special'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'fco-daily-special');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'fco-daily-special'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'fco-daily-special');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'fco-daily-special'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'fco-daily-special') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'fco-daily-special');
            case 'false':
                return __('false', 'fco-daily-special');

            case 'Administrator':
                return __('Administrator', 'fco-daily-special');
            case 'Editor':
                return __('Editor', 'fco-daily-special');
            case 'Author':
                return __('Author', 'fco-daily-special');
            case 'Contributor':
                return __('Contributor', 'fco-daily-special');
            case 'Subscriber':
                return __('Subscriber', 'fco-daily-special');
            case 'Anyone':
                return __('Anyone', 'fco-daily-special');
        }
        return $optionValue;
    }
